...
## State & layout reference (relocated from AGENTS.md §2, 2026-07-29 org trim)

_The operational-home layout + volatile-state-file reference that used to live inline in the always-loaded AGENTS.md. Moved here so the crew file stays lean and this doc is its single owner (as it already claimed to be). Section numbers below refer to the old AGENTS.md structure._
## 2. Layout and state
...
state/               volatile runtime signals; gitignored
  ...
  buzz-replay/       loopback Buzz publisher replay and quarantine state; mechanics are owned by bin/fm-buzz-publish.mjs, with rationale in docs/buzz-loopback-adapter.md

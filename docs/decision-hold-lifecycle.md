...
The no-route form records the same resolution and routed-work markers with an explicit none identity and human-readable none entry before marking an active hold Done.
...
A failed already-Done repair leaves the hold Done and safe to retry.
...
No-route and already-Done repair regression verification date: 2026-08-19.

The focused end-to-end regression uses only synthetic `sample` identities and decision text.
It begins with a completed investigation and visual review whose genuine unresolved choice exists only in the report.
The initial Bearings snapshot correctly has no open decision, and the new teardown gate refuses to erase the source.
A later regression covers tasks-axi's quoted multi-entry `blocked_by` output so `resolve` matches the first, middle, and last ids and rejects a genuinely absent id.
The no-route regressions cover normal active-hold closure, body recording, exact retry, verification, repair of the known hand-closed shape, and refusal to repair a completed captain task without retained captain-hold identity.
...
ok - active captain holds close, retry, and verify with no routed work
ok - declined holds repair only retained captain-held identities

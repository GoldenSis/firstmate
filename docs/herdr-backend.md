...
Provisioning an isolated lab first records the live default session as its fleet-state tripwire, so a machine with `herdr` installed but never started cannot provision one at all.
`tests/herdr-test-safety.sh`'s `herdr_lab_environment_ready` gates on `herdr`, `jq`, and that readable default-session state, so the real-herdr suites skip with an explicit reason instead of failing on an unconfigured machine.

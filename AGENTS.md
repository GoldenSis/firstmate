...
- `wedge-autonudge` - load on a `stale:` wake before the recovery playbook, to try the bounded deterministic nudge first; skip it when the pane is visibly confused, looping, or asking a question.
...
GitHub Issues via `gh` (origin). See `docs/agents/issue-tracker.md`.

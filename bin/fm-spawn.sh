...
model_flag_for_harness() {
  ...
  # Local cost pin (GoldenSis): pi crewmates default to the cheap OpenRouter model, never bare
  # `pi` (which bills Codex/ChatGPT). Override with an explicit --model or FM_PI_DEFAULT_MODEL.
  if [ "$harness" = pi ] && { [ -z "$model" ] || [ "$model" = default ]; }; then
    model=${FM_PI_DEFAULT_MODEL:-openrouter/deepseek/deepseek-v3.2}
  fi

...
# bin/fm-buzz-lib.mjs owns the encoding, while bin/fm-pr-lib.sh owns task-id
# validity; a rejected id exits non-zero here rather than falling back to some
# other channel.
fm_buzz_crew_channel_label() {  # <script dir> <fleet label> <task id>
  ...
  if ! ( . "$script_dir/fm-pr-lib.sh"; fm_task_id_creation_valid "$task_id" ); then
    printf 'rejected crew task id: %s\n' "$task_id" >&2
    return 1
  fi

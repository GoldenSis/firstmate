...
The hold remains the authoritative Captain's Call item until the captain's answer is durably recorded and `bin/fm-decision-hold.sh resolve` closes it through one of two truthful outcomes.
When the answer creates dependent work, create that work in the same backlog, block it by the hold, and resolve with every routed task so the command clears those dependency edges before closing the hold.
When the answer creates no dependent work, such as a decline, a not-now decision, or a duplicate answered by another hold, resolve with `--routed-none` as an explicit no-work attestation.
Never use `--routed-none` to avoid naming dependent work that exists.
...
6. After the captain decides, record any dependent work with normal tasks-axi commands and block it by the hold identity.
7. Put the captain's exact durable decision and rationale in a file, then use the script's `resolve` command with every routed task or with `--routed-none` only when the answer routes to no dependent work.
